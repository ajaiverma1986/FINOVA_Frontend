...
import { AppMgrService, type UpdateMenuPermissionRequest } from '../../services/AppMgrService';
...
type MenuPermissionForm = UpdateMenuPermissionRequest;
...
const PAGE_SIZE = 20;
const emptyForm: MenuPermissionForm = {
  MenuPermissionID: 0,
  MenuID: 0,
  PermissionID: 0,
  Status: 1,
...
function rowId(row: Row) {
  return numberValue(value(row, 'MenuPermissionID'));
}
function toForm(row: Row): MenuPermissionForm {
  return {
    MenuPermissionID: rowId(row),
    MenuID: numberValue(value(row, 'MenuID')),
    PermissionID: numberValue(value(row, 'PermissionID')),
    Status: numberValue(value(row, 'Status')),
  };
}

export default function MenuPermissionMasterComponent() {
  const cache = useQueryClient();
  const [activeOnly, setActiveOnly] = useState(false);
  const [mode, setMode] = useState<Mode>(null);
  const [selectedId, setSelectedId] = useState<number | null>(null);
  const [form, setForm] = useState<MenuPermissionForm>(emptyForm);
  ...
  const list = useQuery({
    queryKey: ['menu-permission-master', activeOnly],
    queryFn: async ({ signal }) =>
      records(
        (
          await (activeOnly
            ? AppMgrService.getActiveMenuPermissions(signal)
            : AppMgrService.getAllMenuPermissions(signal))
        ).Result,
      ),
    retry: false,
  });
  const menus = useQuery({
    queryKey: ['menu-master', false],
    queryFn: async ({ signal }) => records((await AppMgrService.getAllMenus(signal)).Result),
    retry: false,
  });
  ...
  const gridRows = useMemo(() => {
    const menuNames = new Map(
      (menus.data ?? []).map((row) => [
        numberValue(value(row, 'MenuID')),
        value(row, 'Title') ?? value(row, 'MenuName'),
      ]),
    );
      ...
      StatusName:
        ...
        (String(value(row, 'Status')) === '1'
          ...
          : String(value(row, 'Status')) === '0'
            ? 'Inactive'
            : undefined),
      MenuName: value(row, 'MenuName') ?? menuNames.get(numberValue(value(row, 'MenuID'))),
      PermissionName:
        value(row, 'PermissionName') ??
        permissionNames.get(numberValue(value(row, 'PermissionID'))),
    }));
  }, [list.data, menus.data, permissions.data]);
  const detail = useQuery({
    queryKey: ['menu-permission-master', 'detail', selectedId],
    enabled: selectedId !== null && mode !== 'create',
    queryFn: async ({ signal }) =>
      records((await AppMgrService.getMenuPermissionById(selectedId!, signal)).Result)[0],
    retry: false,
  });
  const remove = useMutation({
    mutationFn: (id: number) => AppMgrService.deleteMenuPermission(id),
    onSuccess: async () => {
      setMode(null);
      setSelectedId(null);
      setMessage('Menu permission deleted.');
      await cache.invalidateQueries({ queryKey: ['menu-permission-master'] });
    },
    onError: setError,
  });
  useEffect(() => {
  ...
  function updateField<K extends keyof MenuPermissionForm>(
    field: K,
    fieldValue: MenuPermissionForm[K],
  ) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      if (mode === 'create') {
        const { MenuID, PermissionID, Status } = form;
        await AppMgrService.createMenuPermission({ MenuID, PermissionID, Status });
      } else await AppMgrService.updateMenuPermission(form);
      setMessage(mode === 'create' ? 'Menu permission created.' : 'Menu permission updated.');
      setMode(null);
      setSelectedId(null);
      await cache.invalidateQueries({ queryKey: ['menu-permission-master'] });
    } catch (caught) {
  ...
  return (
    <section className="card master-data-page">
      ...
      {menus.isError && <ErrorState error={menus.error} retry={() => void menus.refetch()} />}
      {permissions.isError && (
      ...
      {!list.data.length ? (
        ...
        <>
          <div className="master-table-wrap">
            <table aria-label="Menu permissions">
              <thead>
                <tr>
                  {columns.map((column) => (
                    <th
                      ...
                      <td>
                        <div className="master-actions">
                          ...
                          <button
                            ...
                            title="Delete menu permission"
                            aria-label="Delete menu permission"
                            onClick={() => {
          ...
          {!!visibleRows.length && (
            <nav
              ...
              aria-label="Menu permission pagination"
            >
              <span>
      ...
      {mode && (
        <CreatePanelDialog create={mode === 'create'} view={mode === 'view'} title={mode === 'view' ? 'Menu permission details' : 'Create menu permission'} onClose={() => setMode(null)}><section className="card gateway-editor" aria-label={`${mode} menu permission`}>{mode === 'create' && error != null && <ErrorState error={error} />}
          ...
          {detail.isError && mode !== 'create' ? (
            ...
            <form className="master-form" onSubmit={submit}>
              <div className="master-form-grid">
                {mode === 'edit' && (
                  <label className="master-field">
                    Menu permission ID
                    <input type="number" readOnly value={form.MenuPermissionID} />
                  </label>
                )}
                <label className="master-field">
                  Menu ID
                  <input
                    ...
                    required
                    value={form.MenuID}
                    onChange={(event) => updateField('MenuID', Number(event.target.value))}
                  />
                </label>
                <label className="master-field">
                  Permission ID
              ...
              <button type="submit" disabled={saving}>
                {mode === 'create' ? 'Create menu permission' : 'Update menu permission'}
              </button>
            </form>
          )}
